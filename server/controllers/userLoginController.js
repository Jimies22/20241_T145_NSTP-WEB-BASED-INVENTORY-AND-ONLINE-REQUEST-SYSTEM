...
const createUser = async (req, res) => {
  ...
  try {
    ...
    const lastUser = await findOne().sort({ userID: -1 }); // Sort by userID in descending order
...
const getAllUsers = async (req, res) => {
  try {
    const users = await find(); // Retrieve all users
    res.json(users);
  } catch (error) {
...
const getUserById = async (req, res) => {
  try {
    const user = await findOne({ userID: req.params.userID });
    if (!user) {
...
const searchUsersByName = async (req, res) => {
  try {
    const users = await find({ name: new RegExp(req.params.name, "i") });
    res.json(users);
  } catch (error) {
...
const deleteUser = async (req, res) => {
  try {
    const user = await findOneAndDelete({ userID: req.params.userID });
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }
