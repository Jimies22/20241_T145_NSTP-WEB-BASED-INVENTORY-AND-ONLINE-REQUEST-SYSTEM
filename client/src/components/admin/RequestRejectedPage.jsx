  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const rejectedRequests = response.data
        .filter(request => request.status === "rejected" && !request.isArchived)
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.status === 200) {
          await logActivity('ARCHIVE_REQUEST', `Archived rejected request for ${itemIdToNameMap[request.itemId] || 'Unknown Item'}`);
              ...
              <div className="modal-body">
                <div className="request-details">
                  ...
                  {selectedRequest.rejectReason && (
                    <p><strong>Rejection Reason:</strong> {selectedRequest.rejectReason}</p>
                  )}
                </div>
              </div>
