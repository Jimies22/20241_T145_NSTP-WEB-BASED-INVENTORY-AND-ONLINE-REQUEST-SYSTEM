...
const ProjectorGrid = ({ searchTerm, selectedFilters }) => {
  ...
  const filteredProjectors = projectors.filter((projector) => {
    const matchesSearch = projector.name
      .toLowerCase()
      .includes(searchTerm?.toLowerCase() || "");
    const matchesFilter = selectedFilters?.length
      ? selectedFilters.includes(projector.status.toLowerCase())
      : true;
  ...
  return (
    <div className="projector-container">
      ...
      {showOverlay && selectedProjector && (
        <BorrowOverlay item={selectedProjector} onClose={handleCloseOverlay} />
      )}
    </div>
  );
};
